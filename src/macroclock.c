...
static bool app_create(int width, int height, void* user_data)
{
	 ...
	 */
	dlog_print(DLOG_INFO, LOG_TAG, "Creating App!");
...
static void app_time_tick(watch_time_h watch_time, void* user_data)
{
	...
	dlog_print(DLOG_INFO, LOG_TAG, "time: %d:%d:%d", s_info.hour, s_info.minute, s_info.second);
...
void app_ambient_tick(watch_time_h watch_time, void* user_data)
{
	...
	dlog_print(DLOG_INFO, LOG_TAG, "(ambient) time: %d:%d:%d", s_info.hour, s_info.minute, s_info.second);
...
{
	dlog_print(DLOG_INFO, LOG_TAG, "main1");
	int ret = 0;
